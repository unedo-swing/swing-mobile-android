...
class DrivingRangeFlow(BaseFlow):
    ...
    # ========================================================================
    # Actions — one screen action per function, composed in the test.
    # ========================================================================
    ...
    def choose_region(self, region: str):
    ...
    def open_swing_credit_history(self):
        ...
        self.history_credit.verify_screen()

    # --- explore / search ---
    def open_search(self):
        self.explore.tap_search()
        self.explore.wait_for(2)

    def search_driving_range(self, range_name: str):
        self.explore.type_search(range_name)
        self.explore.verify_range_visible(range_name)

    def open_result(self, range_name: str):
    ...
    def choose_date(self, range_name: str, date: str):
        # the page opens the calendar from the range's own date button and
        # picks the day in one step
        self.details.select_date(range_name, date)

    def choose_bay_type(self, bay_name: str):
        self.details.select_bay_tab(bay_name)
        self.details.verify_slots_loaded()

    def choose_time(self, time_start: str, time_end: str):
    ...
    def remove_promo(self):
        if self.booking_confirmation.verify_promo_auto_applied() != NO_PROMO_LABEL:
            self.open_promos()
            self.promo.remove_promo()
            self.back_to_confirmation()

    def apply_promo(self, promo_name: str):
        if promo_name not in self.booking_confirmation.verify_promo_auto_applied():
            self.open_promos()
            self.promo.apply_promo(promo_name)
            self.booking_confirmation.verify_screen()   # wait for return

    def apply_and_redeem_promo(self, promo_name: str, promo_code: str):
    ...
    def choose_payment_method(self, payment_method: str):
    ...
    def choose_payment_card(self, card_name: str):
        self.booking_confirmation.tap_select_payment()
        self.payment_method.verify_screen()
        self.payment_method.select_card(card_name)
        self.booking_confirmation.verify_screen()   # wait for return

    def get_payment_information_before_payment(self, bay_type: str = "", used_credit: str = "") -> dict:
    ...
    # --- booking success ---
    def get_booking_code_after_payment(self) -> str:
        return self.success.verify_booking_id()
    ...
    # Verifications
    # ========================================================================
    def verify_exclusive_featured_promo_swing_pass_member(self):
        self.details.verify_exlusive_promo_swing_pass_member()

    def verify_exclusive_featured_promo_regular_member(self):
        self.details.verify_exlusive_promo_regular_member()

    def verify_featured_promos(self) -> list:
        self.featured_promos.verify_auto_claim_banner()
        return self.featured_promos.get_promo_names()

    def verify_maximum_bays_driving_range(self, bays: str):
        self.bays.set_bays(bays)
        self.bays.verify_max_bays(bays)

    def verify_minimum_balls(self, addons: list[dict] | None = None):
    ...
    def verify_booking_information(self, date: str, time_start: str, time_end: str,
                                   bays: int, bay_type: str):
    ...
    def verify_payment_success_driving_range(self, payment_information: dict,
                                             bay_type: str = "") -> dict:
        """The confirmed booking, field by field against what the booking
        confirmation showed before paying. Returns the confirmed summary."""
        after = self.success.get_summary(bay_type)

        mismatches = compare_booking(payment_information, after)
        ...
        after["credit_used"] = payment_information.get("credit_used", "")
        after["booking_id"] = self.get_booking_code_after_payment()
        return after
    ...
    def verify_used_credit_booking_code(self, booking_code: str, total_credits: str):
        self.history_credit.tap_filter_credit_used()
        self.history_credit.verify_credit(booking_code, total_credits)

    def verify_earned_credit_booking_code(self, booking_code: str, total_credits: str):
        self.history_credit.tap_filter_credit_earning()
        self.history_credit.verify_credit(booking_code, total_credits)

    # ========================================================================
    # Composed steps — several actions above in one call. Kept for the suites
    # that drive the flow screen by screen (the regular-player, Malaysia and
    # cross-country driving range tests).
    # ========================================================================
    ...
    def search_and_open_range(self, range_name: str):
        ...
        self.explore.verify_range_visible(range_name)
        self.open_result(range_name)

    def select_schedule(self, name: str, date: str, bay_name: str,
                        time_start: str, time_end: str):
        self.choose_date(name, date)
        self.choose_bay_type(bay_name)
        self.choose_time(time_start, time_end)

    def book_and_set_bays(self, bays: int):
        self.book_driving_range()
        self.set_bays(bays)
        self.confirm_bays()

    def verify_maximum_bays(self, bays: str):
        self.book_driving_range()
        self.verify_maximum_bays_driving_range(bays)

    def verify_booking_confirmation_details(self, date: str, time_start: str,
                                            time_end: str, bays: int, bay_type: str):
        self.verify_booking_information(date, time_start, time_end, bays, bay_type)
    ...
    def change_payment_card(self, card_name: str):
        self.choose_payment_card(card_name)

    def change_promo_with_add_promo_code(self, promo_name: str, promo_code: str):
        self.apply_and_redeem_promo(promo_name, promo_code)

    def get_data_before_payment(self, bay_type: str = "", used_credit: str = "") -> dict:
        return self.get_payment_information_before_payment(bay_type, used_credit)

    def pay_and_get_confirmed_booking(self, before: dict, bay_type: str = "") -> dict:
        self.pay_now()
        self.proceed_to_pay()
        return self.verify_payment_success_driving_range(before, bay_type)

    def go_back_to_activity(self):
        self.back_to_activity()
    ...
    def verify_featured_promo_exclusive_swing_pass_member(self):
        self.verify_exclusive_featured_promo_swing_pass_member()
        self.open_featured_promos()
        self.verify_featured_promos()
        self.back_to_details()

    def verify_featured_promo_exclusive_regular_member(self):
        self.verify_exclusive_featured_promo_regular_member()
        self.open_featured_promos()
        self.verify_featured_promos()
        self.back_to_details()
    ...
    def verify_swing_credit_usaged(self, code_booking: str, tot_credits: str):
        ...
        self.open_swing_credit_history()
        self.verify_used_credit_booking_code(code_booking, tot_credits)
