...
@pytest.mark.android
class TestDrivingRangeBookingCrossCountry:

    @pytest.mark.app_state("force-stop")
    @pytest.mark.skip
    @pytest.mark.parametrize("TC_ID", ["DR_CC_001"])
    def test_end_to_end_booking_driving_range_without_promo_id_to_my(self,TC_ID, login_flow, driving_range_flow):
        D.load(TC_ID)
    ...
    def test_end_to_end_booking_driving_range_without_promo_used_credit_id_to_my(self,TC_ID, login_flow, driving_range_flow):
        D.load(TC_ID)
        ADD_ONS = load_add_ons(TC_ID)
        pdf = init_pdf(D.TC_NAME, tc_id=TC_ID)
        login_flow.verify_home()
    ...
    @pytest.mark.parametrize("TC_ID", ["DR_CC_003"])
    def test_end_to_end_booking_driving_range_without_promo_my_to_id(self,TC_ID, login_flow, driving_range_flow):
        D.load(TC_ID)
        ADD_ONS = load_add_ons(TC_ID)
        pdf = init_pdf(D.TC_NAME, tc_id=TC_ID)
        login_flow.verify_home()
    ...
    @pytest.mark.app_state("force-stop")
    @pytest.mark.regression
    @pytest.mark.parametrize("TC_ID", ["DR_CC_004"])
    def test_end_to_end_booking_driving_range_without_promo_used_credit_my_to_id(self,TC_ID, login_flow, driving_range_flow):
        D.load(TC_ID)
        ADD_ONS = load_add_ons(TC_ID)
        pdf = init_pdf(D.TC_NAME, tc_id=TC_ID)
        login_flow.verify_home()
